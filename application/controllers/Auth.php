...
class Auth extends MY_Controller
{
    ...
    public function index()
    {
        if( $this->require_min_level(1))
        {
            ...
            $pass_update_date = $this->mcommon->find_pass_update_date($user_id);
            $view_data['password_update_date'] = $pass_update_date;
            $view_data['Current_Month_sales']           = $this->mpurchase->Current_Month_sales();
            $view_data['Current_Month_purchase']        = $this->mpurchase->Current_Month_purchase();
            $view_data['Outstanding_Receivable_pay']    = $this->mpurchase->menu_terms_report();
            $view_data['Outstanding_Receivable_sal']    = $this->mpurchase->menu_terms_sales_report();
            $view_data['Outstanding_Payable_pay']       = $this->mpurchase->Outstanding_Payable_pay();
            $view_data['Outstanding_Payable_paid']      = $this->mpurchase->Outstanding_Payable_paid();
            $view_data['Stock_value']                   = $this->mpurchase->Stock_value();
            $view_data['reorder']                       = $this->mpurchase->reorder();
            // $view_data['topTenSalesItem']               = $this->pre->getTopTenSalesItem();
            // $view_data['topTenMovingItem']              = $this->pre->getTopTenMovingItem();
            // $view_data['groupWiseSales']                = $this->pre->getGroupWiseSales();
     ...
     * Remember to add "Auth/optional_login_test" to the
    ...
    public function create_user()
    {
        ...
        */
        $user_data = array(
            'username'   => 'admin',
            'passwd'     => 'BrightHouse!23',
            'email'      => 'admin@agmtechnical.com',
            'auth_level' => '9', // 9 if you want to login @ Auth/index.
        );
    ...
    public function recover()
    {
        ...
        $data=array(
                        'content'   =>$this->load->view('Auth/recover_form', ( isset( $view_data ) ) ? $view_data : '', TRUE )
                   );
    ...
    public function ajax_login()
    {
        ...
        $html = $this->load->view('Auth/page_header', $data, TRUE);
        ...
        if(isset($_POST['submit']))
            {
                 ...
                 if($this->form_validation->run() == TRUE) 
                {
                   ...
                   $user_id =  $this->mcommon->find_email_user_id($email); 
                    if($user_id=='')
                    {
                        $sessionArr             = $this->session->all_userdata();
                        $this->session->set_userdata('successMsg', 'No user found in this Email ID....');
                        $view_data['notification']   = $sessionArr['successMsg'];
                         
                         $data = array(
                        'title'     => "Wholesale login",
                        'content'   =>$this->load->view('Auth/forgot_password',$view_data,TRUE)
                        );
                         $this->load->view('templates/forgot_pass_template', $data);
                    }
                    else
                    {
                        $userEx_pass = $this->mcommon->find_user_last_password($user_id);
                        $data['organization_detail']    =   $this->mcommon->getCompanyProfiles('1');
                        $data['old_password']= $userEx_pass;
                    ...
                    $content = 'Please use below  password login '.$c_org_name.' App  <br/><br><br><br>
                     Password    :   '.  $userEx_pass.'<br>
                     (Password is case sensitive)<br>';
                    $body = 'PRIVATE & CONFIDENTIAL <br/><br/>&nbsp;&nbsp;&nbsp;&nbsp;';
                    $body.= $content.'<br/><br/>';
                    $body.= $this->load->view('Auth/forgot_password_email', $data, TRUE);
                    ...
                    if($this->email->send()) 
                    {
                         ...
                         $view_data['notification1']   = $sessionArr['successMsg'];     
                    ...
                    {
                        $sessionArr             = $this->session->all_userdata();
                        $this->session->set_userdata('successMsg', 'Error in sending Email....');
                         $view_data['notification']   = $sessionArr['successMsg'];      
