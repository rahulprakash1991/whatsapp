...
class Proforma extends MY_Controller {
	...
	{  
		if($this->require_min_level(1))
        {
			...
			$inv_prifix = $this->sal_common->GetProformaNoprefix();
			$inv_num =$this->sal_common->GetProformanumber();
			$inv_draf_sufix = $this->sal_common->GetProformaNosuffix();
...
// 		$this->load->view('proforma_invoice/print_invoicesARA1', $data);
			...
			$pdf->AddPage();
			$html=$this->load->view('proforma_invoice/TCPDFwithoutHeader',$data, TRUE);
			$pdf->writeHTML($html, true, false, true, false, '');
			ob_end_clean();
			$pdf->Output('ProformaReport.pdf', 'I');
...
// 		$this->load->view('proforma_invoice/print_invoicesARAwithHeader', $data); 
			...
			$pdf->SetFont('dejavusans', '', 10);
			$pdf->AddPage();
			$html=$this->load->view('proforma_invoice/TetPDF',$data, TRUE);
			$pdf->writeHTML($html, true, false, true, false, '');
			ob_end_clean();
			$pdf->Output('ProformaReport.pdf', 'I');
...
public function addClientRep()
{
	$view_data['id'] 				=  $this->input->get('invoice_id');
 	echo $this->load->view('proforma_invoice/print_modal',$view_data,TRUE);	
