				...
				else
				{
					...
					if($this->input->post('client_id')!='')
					{
						$value_array	=	array(
													...
													'client_curency' => $this->input->post('client_curency'),
							...
							foreach($addressEngArr as $key => $val)
							{
								if($idArr[$key]!='')
								{
									...
									$value_array1	=	array
														(	
															'client_id'		    =>	$this->input->post('client_id'),
															'client_cr_no'      =>$cr_noEngArry[$key],
															'client_vat_no'      =>$vat_num_engArray[$key],
															'client_address'	=>	$addressEngArr[$key],
															'client_street'     =>$street_engArray[$key],
															...
															'client_cr_no_arabic'      =>$cr_noArabicArry[$key],
															'client_vat_no_arabic'      =>$vat_num_ArabicArray[$key],
															
															'client_street_arabic'     =>$street_Arabic_Array[$key],
...
// print_r($value_array);die;
	...
	public function getPartNoContent()
	{
		...
		echo $this->load->view('masters/client/add_ship_address',$msg,TRUE);
	}
...
public function deleteshipAddress()
	{
		$res = $this->mcommon->common_delete('table_client_address',array('id' => $this->input->get('prid') ));
	}
