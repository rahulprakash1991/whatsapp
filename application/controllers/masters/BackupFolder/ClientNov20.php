...
class Client extends MY_Controller {
	...
	{  
		if($this->require_min_level(1))
        {
			...
			{
				$this->table->set_heading('Operation','Client Key','Name','Abbrevation','Client Representative','Assign  AS Supplier' ,'Email', 'Mobile', 'Website'); 
			}
	...
	function datatable()
    {
   $this->datatables->select('c.id,c.client_no,c.client_name,c.client_abb,c.id as rep,c.assign_vender,c.client_email,c.client_mobile, c.client_website,c.id as client_id')
		...
		if($this->require_min_level(1))
        {
			...
			$msg['notification'] 	= $sessionArr['successMsg'];
 			...
 			$data=array(
							...
							'content'   =>	$this->load->view('masters/client/add_client_test',$msg,TRUE)
						);
	...
	public function add()
	{
		...
		{	
			if(isset($_POST['Submit']))
			{
				...
				$this->form_validation->set_rules('client_arabic_name','اسم  ', 'required');
				...
				else
				{
					...
					if($this->input->post('client_id')!='')
					{
						$value_array	=	array(
													...
													'client_ab_arabic' =>$this->input->post('client_ab_arabic'),
													...
													'client_area1'				=>	$this->input->post('area1'),
													'client_city1'				=>	$this->input->post('city1'),
													'client_state1'				=>	$this->input->post('state1'),
													'client_zip1'				=>	$this->input->post('zip1'),
													'client_country1'				=>	$this->input->post('country1'),
