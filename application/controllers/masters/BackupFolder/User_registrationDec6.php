...
class User_registration extends MY_Controller {
	...
	function datatable()
    {
    	...
    	if(($log_user_role=='10') ||($log_user_role=='9' )  ){
    	$this->datatables->select('b.user_id, b.username,b.user_id as U_id,b.user_id as lock_id,b.banned, r.role_name, b.email, b.username AS s, b.created_at, b.updated_by, b.modified_at')
	        ->from('users AS b')
    ...
    {
    	$this->datatables->select('b.user_id, b.username, r.role_name, b.email, b.username AS s, b.created_at, b.updated_by, b.modified_at')
	        ->from('users AS b')
	        ->join('tbl_user_role AS r', 'r.role_id=b.auth_level', 'left')
	      	// ->where('b.banned','0')
			->edit_column('b.user_id', get_buttons_new('$1','masters/User_registration/', 'b.user_id'), 'b.user_id');
			...
			{			
				if($this->input->post('user_id') > 0)
				{
					...
					if( $this->form_validation->run() )
					{
						...
						//$user_data['banned']		=	$this->input->post('banned');
	...
	public function AdminResetPassword()
	{
		...
		$data = $this->input->get('data');
			...
			$e_host='ssl://smtp.googlemail.com';
					...
					$data=['use_name'=>$user_email, 'sys_pass'=> $data];
				
					$to_email =   strip_tags($data['use_name']);
					$this->load->library('email');
					$this->email->initialize($config);
					$this->email->from('rahulrithu2016@gmail.com');
					// $list = array($c_org_name);
