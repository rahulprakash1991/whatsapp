...
class Proforma extends MY_Controller {
	...
	public function manage()
	{
		if($this->require_min_level(1))
        {
        	...
        	{
        		$msg['datatable_url']	=	'Proforma/datatable/'.$from_date1.'/'.$to_date1.'/'.$vendor_id.'/'.$msg['payment_status'].'/'.$msg['inv_status'];
        	}
        	else
        	{
        		$msg['datatable_url']	=	'Proforma/datatable';
			...
			$this->table->set_heading('Operation', 'Proforma Number', 'Proforma Date', 'Client Name', 'Sales Amount', 'Proforma Status', 'Updated By', 'Updated On'); 
	...
	function datatable($from_date1='', $to_date1='', $vendor_id='', $payment_status='', $inv_status='')
    {
		...
		$this->datatables->edit_column('b.per_sal_invoice_status', '$1', 'get_status1("b.per_sal_invoice_status")');
				...
				if($this->form_validation->run() == TRUE) 
				{
					if($this->input->post('sal_id')!='')
					{
						...
						$value_array 	= array(
													...
													'per_sal_order_date'			=> $this->input->post('sal_order_date'),
