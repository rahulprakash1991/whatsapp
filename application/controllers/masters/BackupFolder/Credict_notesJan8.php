...
class Credict_notes extends MY_Controller {
	...
	{  
		if($this->require_min_level(1))
        {
	        ...
	        $msg['form_toptittle']		=	"Credit Notes";
	...
	function datatable()
    {
        // $this->datatables->select('cn.credict_id,cn.credit_number,cn.credict_id as attac,c.client_name,s.sal_order,u.username, cn.credictnote_createdOn,v.username AS s,cn.credictnotes_updatedOn,u.username')
           $this->datatables->select('cn.credict_id,cn.credit_number,cn.credictnote_createdOn,c.client_name,s.sal_order,s.sal_created_on,cn.balance_amount,cn.credict_total,cn.balance_amount as balance')
	...
	{  
		...
		if($this->require_min_level(1))
        {
	        ...
	        $msg['form_toptittle']	=	'Credit Notes Management';
        	$msg['list_tittle']		=	'Credit Notes list';
			...
			if(isset($_POST['Submit']))
			{
				...
				if($this->form_validation->run() == TRUE) 
				{
					...
					else
					{
						...
						$value_array 	= array(
													'client_id'			=> $this->input->post('sal_company_name'),
													'credit_number'  =>$credit_order_performanum,
													'reason'                =>    $this->input->post('reason'),
													'subject'                =>    $this->input->post('subject'),
													'salesorder_id' =>($this->input->post('sal_client_rep')!='') ? ($this->input->post('sal_client_rep')) : '',	
													'credictnote_createdBy'			=>  $this->auth_user_id,
...
public function printCredict_notes($id='' )
{
	...
	$this->load->view('credict_notes/print_cr_notes', $data); 

}
public function get_credit_no()
{
	$invoiceNumber 	= 	$this->sal_common->getCriditNoteNo();
	echo $invoiceNumber;
}
...
*/
 	...
 	public function applyInvoice($credit_id ='',$client_id ='' )
	{
        ...
        if(isset($_POST['submit']))
        {
			...
			foreach($inv_idArr as $key => $val)
			{
				if($inv_idArr[$key]!='')
					{
					$invoice_paid_amount = $this->mcommon->find_invoice_paid_amount($inv_idArr[$key]);
					$new_paid_amount = Number($invoice_paid_amount) + Number($amount_crtArr);
					$value_arraytotal 	=	array(
															'paid_amount'		     =>	$new_paid_amount,
															
												);
							
					$where_arraytotal 	=	array('sal_id' =>$inv_idArr[$key]);
