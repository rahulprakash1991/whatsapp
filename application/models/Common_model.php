...
class Common_model extends CI_Model
{
	...
	*/
	...
	{
		$data = array();
		$this->db->select('s.id,s.vendor_id,s.vendor_cr_no,s.vendor_vat_no,s.vendor_address,s.vendor_street,s.district,s.area,s.city,s.state,s.post_code,s.country,s.phone,s.phone1,s.mobile,s.email,s.fax,s.website,s.vendor_cr_no_arabic,s.vendor_vat_no_arabic,s.vendor_street_arabic,s.district_arabic,s.vendor_address_arabic,s.area_arabic,s.city_arabic,s.state_arabic,s.post_code_arabic,s.country_arabic,s.phone_arabic,s.phone1_arabic,s.mobile_arabic,s.email_arabic,s.fax_arabic,s.website_arabic')->from('table_vendor_address AS s')->where('s.vendor_id', $id);
		$query  = $this->db->get();
		$result = $query->result();
		foreach ($query->result() as $row) {
			$data[] = $row;
	...
	{
		$data = array();
		$this->db->select('s.id,s.client_address,s.area,s.city,s.state,s.post_code,s.country,s.phone,s.mobile,s.email,s.fax,s.website,s.client_address_arabic,s.area_arabic,s.city_arabic,s.state_arabic,s.post_code_arabic,s.country_arabic,s.phone_arabic,s.mobile_arabic,s.email_arabic,s.fax_arabic,s.website_arabic,s.client_cr_no,s.client_vat_no,s.client_street,s.district,s.phone1,s.client_cr_no_arabic,s.client_vat_no_arabic,s.client_street_arabic,s.district_arabic,s.phone1_arabic')->from('table_client_address AS s')->where('s.client_id', $id);
		$query  = $this->db->get();
		$result = $query->result();
		foreach ($query->result() as $row) {
			$data[] = $row;
    ...
    $quote_id = $this->db->select_max('quo_id')->where('enq_no',$id)->limit(1)->get('quotation')->row('quo_id');
 ...
 //    return $update_date;
  ...
  //   	$result =  	$ci->db->get()->result();
	...
	function VendorRepdetails($id = '')
	{
		
		$this->db->select('cr.id,cr.rep_name,cr.email,cr.mobile,d.desgination_name')
		->from('tbl_vendor_rep AS cr')	
		->join('tbl_desgination d','d.desgination_id = cr.designation','left')
		
		->where('cr.vendor_id',$id);		
	...
	//march 3
	public function find_client_id($id)
	{

    $enq_id = $this->db->select_max('client_id')->where('id',$id)->limit(1)->get('tbl_enquiry')->row('client_id');
   
    return $enq_id;    
	}
	public function find_client_id_qus($id)
	{

    $enq_id = $this->db->select_max('quotation_vendor')->where('quo_id',$id)->limit(1)->get('quotation')->row('quotation_vendor');
