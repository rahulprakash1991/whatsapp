...
class Sal_common_model extends CI_Model
{
    ...
    {
		$this->db->select('client_id,rep_name,id');		
	 ...
	 function drop_menu_vendor_rep($id = '')
    {
		$this->db->select('vendor_id,rep_name');		
		$this->db->order_by('rep_name', 'ASC');
		if($id)
		{
		...
		$query=$this->db->get('tbl_vendor_rep');
		$result = $query->result();
		$options[''] = '-- Select --' ;
		foreach($result as $item)
		{
     ...
     function drop_menu_vendor_address($id = '')
    {
		$this->db->select('id,vendor_address,vendor_id');		
		...
		foreach($result as $item)
		{
			$options[$item->id] = $item->vendor_address;
		}
      return $options;
    }
...
$this->db->select('s.sal_item_id,s.sal_id,s.item_description, s.discount,s.vat_per,s.total_amount,s.qty,s.unit_price,s.	vat_amount1')
...
$this->db->select('s.sal_item_id, s.sal_id, s.item_description,s.discount,s.vat_per,s.total_amount,s.qty,s.unit_price,s.vat_amount1')
	...
	{ 
		$data = array();
		$this->db->select('s.qus_item_id,s.qus_id,s.item_description,s.item_description_arabic,s.unit,s.	qty,s.unit_price,s.total_cost,s.dis_per,s.dis_amt,s.vat_per,s.vat_amt')
	...
	function get_company_detail_proforma($id)
	{ 
		
		$this->db->select('s.per_sal_id,s.per_sal_company_name')
		->from('per_sales_order AS s')
		->where('status','1')
		->where('s.per_sal_id',$id);

		$query=$this->db->get()->row_array();
		return $query['per_sal_company_name'];
	}
	...
	public function getInvoiceDraftprefix()
	{
		$pr = $this->db->get("tbl_preferences")->result();
		foreach ($pr as $p) {
						$pre[addslashes($p->key)] = addslashes($p->key_value);
		}
		return $pre['inv_prefix_draft'] ;
	}
	public function getInvoiceDraftnumber()
	{
		$pr = $this->db->get("tbl_preferences")->result();
		foreach ($pr as $p) {
						$pre[addslashes($p->key)] = addslashes($p->key_value);
		}
		return $pre['inv_number_draft'] ;
	}
	public function getInvoiceDraftsuffix()
	{
		$pr = $this->db->get("tbl_preferences")->result();
		foreach ($pr as $p) {
						$pre[addslashes($p->key)] = addslashes($p->key_value);
		}
		return $pre['inv_suffix_draft'] ;
	}
	public function getQuotationNo()
	{
		...
		public function getQuotationprifix()
	{
		$pr = $this->db->get("tbl_preferences")->result();
		foreach ($pr as $p) {
						$pre[addslashes($p->key)] = addslashes($p->key_value);
		}
		return $pre['quo_prefix'] ;
	}
		public function getQuotationNumber()
	{
		...
		return $pre['quo_number'] ;
	}
		public function getQuotationSufix()
	{
		$pr = $this->db->get("tbl_preferences")->result();
		foreach ($pr as $p) {
						$pre[addslashes($p->key)] = addslashes($p->key_value);
		}
		return $pre['quo_suffix'];
	}
	...
	public function GetProformaNoprefix()
	{
		$pr = $this->db->get("tbl_preferences")->result();
		foreach ($pr as $p) {
						$pre[addslashes($p->key)] = addslashes($p->key_value);
		}
		return $pre['pro_inv_prefix'] ;
	}
	public function GetProformanumber()
	{
		...
		return $pre['pro_inv_number'] ;
	}
	public function GetProformaNosuffix()
	{
		$pr = $this->db->get("tbl_preferences")->result();
		foreach ($pr as $p) {
						$pre[addslashes($p->key)] = addslashes($p->key_value);
		}
		return $pre['pro_inv_suffix'];
	}
...
function get_bank_id($id)
	...
	public function getPer_CriditNoteNoprefix()
	{
		$pr = $this->db->get("tbl_preferences")->result();
		foreach ($pr as $p) {
						$pre[addslashes($p->key)] = addslashes($p->key_value);
		}
		return $pre['per_credit_prefix'];
	}
	public function getPer_CriditNoteNonumber()
	{
		$pr = $this->db->get("tbl_preferences")->result();
		foreach ($pr as $p) {
						$pre[addslashes($p->key)] = addslashes($p->key_value);
		}
		return  $pre['per_credit_number'] ;
	}
	...
	public function getCriditNoteNoprefix()
	{
		$pr = $this->db->get("tbl_preferences")->result();
		foreach ($pr as $p) {
						$pre[addslashes($p->key)] = addslashes($p->key_value);
		}
		return $pre['credit_prefix'] ;
	}
	public function getCriditNoteNonumber()
	{
		$pr = $this->db->get("tbl_preferences")->result();
		foreach ($pr as $p) {
						$pre[addslashes($p->key)] = addslashes($p->key_value);
		}
		return $pre['credit_number'] ;
	}
	...
	{ 
		$data = array();
		$this->db->select('cn.credict_item_id,cn.credict_id,cn.item_description,cn.item_description_arabic,cn.qty,cn.unit_price,cn.total,cn.unit')
	...
	public function find_sale_oredr_draft_number($id)
	{
	  $sal_order = $this->db->select_max('sal_order')->where('sal_id',$id )->limit(1)->get('sales_order')->row('sal_order');
    	return $sal_order;
	}
	...
	{ 
		...
		$this->db->select('s.quo_id,s.quotation_vendor')
		->from('quotation AS s')
		->where('status','1')
		->where('s.quo_id',$id);

		$query=$this->db->get()->row_array();
		return $query['quotation_vendor'];
	}
	function get_quotation_bank_id($id)
	{ 
		
		$this->db->select('s.quo_id,s.quotation_bank')
		->from('quotation AS s')
		->where('status','1')
		->where('s.quo_id',$id);

		$query=$this->db->get()->row_array();
		return $query['quotation_bank'];
	}
		public function getDraftQuotationprifix()
	{
		$pr = $this->db->get("tbl_preferences")->result();
		foreach ($pr as $p) {
						$pre[addslashes($p->key)] = addslashes($p->key_value);
		}
		return $pre['dquo_prefix'] ;
	}
		public function getDraftQuotationNumber()
	{
		$pr = $this->db->get("tbl_preferences")->result();
		foreach ($pr as $p) {
						$pre[addslashes($p->key)] = addslashes($p->key_value);
		}
		return $pre['dquo_number'] ;
	}
		public function getDraftQuotationSufix()
	{
		$pr = $this->db->get("tbl_preferences")->result();
		foreach ($pr as $p) {
						$pre[addslashes($p->key)] = addslashes($p->key_value);
		}
		return $pre['dquo_suffix'];
	}
	//April 3
	function drop_menu_client_Currency($id = '')
    {
	    $this->db->select('cur_id,cur_name');		
		$this->db->order_by('cur_name', 'ASC');
		if($id)
    ...
    public function find_client_currency_id($id)
	{
	  $client_curency = $this->db->select_max('client_curency')->where('id',$id )->limit(1)->get('tbl_client')->row('client_curency');
    	return $client_curency;
	}
	 function drop_menu_client_Enquiry_Currency($id = '')
    {
		$this->db->select('cur_id,cur_name');		
		$this->db->order_by('cur_name', 'ASC');
		$this->db->where('cur_id', $id);
      ...
      return $options;
        ...
        //April 5
     function drop_menu_client_rep_invoice($id = '')
    {
		$this->db->select('client_id,rep_name,id');		
		$this->db->order_by('rep_name', 'ASC');
		if($id)
