...
{ 
	...
	{ 
		$data = array();
		$this->db->select('s.pro_item, s.pro_id, p.pro_item_name, p.pro_item_id, p.pieces_per_unit, s.unit, s.price_amt, s.quantity, s.pro_amount as amount, t.tax_id, t.tax_name, t.tax_percent, p.pro_item_stock AS available_qty')
			->from('proforma_invoice_item AS s')
			->join('proforma_invoice so', 'so.pro_id = s.pro_id', 'left')
			->join('tbl_pro_item p', 'p.pro_item_id = s.pro_item_id', 'left')
	...
	{ 
		...
		$this->db->select('s.pro_id,s.pro_company_name')
		->from('proforma_invoice AS s')
		->where('s.pro_id',$id);
		$query=$this->db->get()->row_array();
		return $query['pro_company_name'];
	}
	...
	{ 
		$data = array();
		$this->db->select('s.tbl_tax_id,s.pro_tax_amount,t.tax_name')
			->from('proforman_invoice_tax AS s')
			->join('tbl_tax t','t.tax_id = s.pro_tax_id')
			->where('s.pro_id',$id);
