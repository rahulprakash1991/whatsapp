...
function drop_menu_title()
    ...
    function drop_menu_client( $id = '' )
    {
       ...
       function drop_menu_payment_terms( $id = '' )
    {
		$this->db->select('payment_terms_id, 	payment_terms');
		$this->db->order_by('payment_terms_id', 'ASC');
		$this->db->where('payment_delete_status','1');
		if ($id) {
   	...
   	function drop_menu_productitem()
	{
   	 ...
   	 function drop_menu_invoice_tax_item( $id = '' )
    {
		...
		foreach($result as $item)
		{
			$options[$item->tax_percent] = $item->tax_percent;
		}

    	return $options;
    }
       function drop_menu_client_enquiry( $id = '' )
    {
		$this->db->select('id, client_name');
		$this->db->order_by('id', 'ASC');
		$this->db->where('client_delete_status','1');
     ...
     function drop_menu_client_rep_enquiry($id = '')
    {
    ...
    //March 3
     function drop_menu_client_rep_enq($id)
    {
		$this->db->select('id,rep_name');		
		$this->db->order_by('id', 'ASC');
    ...
    //April 2
        function drop_menu_quotation_validity( $id = '' )
    {
		$this->db->select('	quotation_validity_id,quotation_validity');
		$this->db->order_by('quotation_validity_id', 'ASC');
		$this->db->where('quotation_validity_delete_status','1');
		if ($id) {
			$this->db->where('quotation_validity_id', $id);
		}
		$query=$this->db->get('tbl_quotation_validity');
		$result = $query->result();
		$options[''] = '-- Select --' ;
		foreach($result as $item)
		{
			$options[$item->quotation_validity_id] = $item->	quotation_validity;
		}
