...
foreach($company_detail->result() as $row)
  ...
  $Currency_Name=$this->pre->getCurrencyName($currency)
?>
    <!-- invoice  Start -->
<style>
.break
...
<table width="100%" border="1" rules="all"  style="border-color: black">
  ...
  <td style="width: 32%;text-align: center;"><u><b>CREDICT NOTES/SALES RETURN </b></u></td>
  ...
  <td>Customer VAT No-   مالضريبي للعميل  <br><?php echo $contact_vat ;?>    </td>
...
$number = $cost_total_amount1 ;
  ...
  $points = ($point) ?
    "." . $words[$point / 10] . " " . 
          ...
          // else
          // {
             $grand_amount_word =  $result .  $Currency_Name;
          // }
...
<!--   <td colspan="2" style="font-size: 11px;"><b>Total Amount <br> المبلغ الإجمالي</b>  </td> -->
  <td style="font-size: 13px;text-align: right; padding-right:10px;"><b><?php echo number_format($cost_total_amount1 , 2, '.', '');?></b></td>
  </tr>
 </table>
 <table width="100%" rules="all" border="1">
   <tr>
     ...
     <td style="width: 35%;">Accounts:<br><br>
    Date: </td>
