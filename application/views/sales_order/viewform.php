...
if(isset($value) )
{
		...
		{
			$inv_id 					=	$row->sal_id;
			$inv_client 			=	$row->sal_company_name; 
			$inv_client_rep = $row->sal_client_rep;
			$client_date 	= $row->sal_order_date;
			$refference 	= $row->remarks;
			$payment_term 	= $row->payment_term;
			$client_bank 	= $row->sal_bank;
			$sub_total 		= $row->sal_sub_total;
			$grand_total 	= $row->sal_grand_total;
			$vat_amount 	= $row->sal_tax_amount;
			$remarks 		= $row->sal_general_terms;
			$qus_status = $row->qus_status;
			$dis_amount = $row->sal_discount;
			$client_curency = $row->sal_currency;
...
{	
			...
			$inv_client 			=	$this->input->post('client_name');
			$inv_client_rep 		=   $this->input->post('client_rep');
			$client_date 			=   $this->input->post('client_date');
									 	...
									 	<div class="col-md-3">
				                        	<div class="form-group">
				                        		<div class="form-group <?PHP if(form_error('client_name')){ echo 'has-error';} ?>">
				                                	...
				                                	$attrib = 'class="bootstrap-select" data-live-search="true" data-width="100%" id="client_name" onChange="loadaddress(this.value)"';
													echo form_dropdown('client_name', $drop_menu_client, set_value('client_name', (isset($inv_client)) ? $inv_client : ''), $attrib);
													?>
				                        	...
				                        	<div class="form-group">
				                        		<div class="form-group <?PHP if(form_error('client_rep')){ echo 'has-error';} ?>">
				                                	...
				                                	$attrib = 'class="bootstrap-select" data-live-search="true" data-width="100%" id="client_rep" ';
													echo form_dropdown('client_rep', $drop_menu_client_rep, set_value('client_rep', (isset($inv_client_rep)) ? $inv_client_rep : ''), $attrib);
													?>
													 <label class="error"><?php echo form_error('client_rep'); ?></label>
												</div>
				                      		...
				                      		<div class="col-md-3">
				                        	<div class="form-group">
				                        		<div class="form-group <?PHP if(form_error('client_date')){ echo 'has-error';} ?>">
													<label>Invoice Date</label>
				                               	 	 <input type="text" class="form-control date-picker" placeholder="Invoice Date" autocomplete="off" value="<?php echo ($client_date!='' && $client_date!='0000-00-00') ? date('m/d/Y', strtotime($client_date))  : date('m/d/Y '); ?>" name="client_date" id="client_date"   >
													 <label class="error"><?php echo form_error('client_date'); ?></label>
												</div>
                                       ...
                                       <div class="col-md-1">
                                    	<div class="form-group" >
											...
											<select name="vat_per1[]" id="vat_per1<?php echo $i;?>" class="form-control vat_per1" >
                     						...
                     						{?>
                        					<option value="<?php echo $key_id;?>" <?php if($key_id == $vat_per[$i]){?> selected <?php }?>><?php echo $key_name;?></option>
                            ...
                            <div class="col-md-12">
                                <div class="row">
			                     ...
			                     <?php }?>
					              ...
					              <button type="submit" name="saveinvoice" class="btn btn-success"><?php if($qus_status=='1') { ?><?php echo'Update  Quotation'; ?> <?php }else {?><?php echo'Save As Invoice'; ?><?php }?></button>
					                       <a href="<?php echo base_url().'Sales_order/manage/'?>" class="btn btn-primary"><?php echo $this->lang->line('cancel'); ?></a> 
					                    </div>
					                </div>
           						</div>                         
								<?php echo form_close(); ?>
			...
			function addNewPart()
			{
                ...
                $.ajax({
					...
					function getClientCurrency(id)
					{
							...
							$.ajax({
								type: "GET",
								url: "<?php echo site_url('Sales_order/getClientCurrency'); ?>",
								data: { con_id:id},
								dataType:"html",
								success: function(html)
								{	
								
									$('#client_curency').html(html);
