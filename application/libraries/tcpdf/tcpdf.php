...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	* @todo set customized colors/background
	**/
	function buildHorizGraph($data,$lineHeight=8,$padding=0,$barHeight=6,$gridTextPt=12,$showGrid=true)
	{
		$MAXSW=200/2.54;	// Max string width
		$GR=100/2.54; 	// Graph ratio percentage
		$OW=100/2.54;	// Option cell width
		$GW=350/2.54;	// Graph bar max Width
		$CW=20/2.54;		// Grap CELL width;
		...
		foreach($final_data as $idx=>$d)
		{
			...
			$this->cell($OW,0,$this->stringReduce($d['text'],$MAXSW),0,0,'L');
